...
def get_args():
    ...
    parser.add_argument("--num_iters", type=int, default=120000)
...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        action_batch = torch.from_numpy(np.array(action_batch_tmp, dtype=np.float32))       
        ...
        if iter +1 == opt.num_iters:
            torch.save(model, "{}/ns_shaft_{}".format(opt.saved_path, iter+1))
